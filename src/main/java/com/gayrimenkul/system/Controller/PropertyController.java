...
@RestController
@RequestMapping({"/api/properties", "/api/listings"})
...
public class PropertyController {
    ...
    @GetMapping
    public ResponseEntity<List<Property>> getAllProperties(@RequestParam(required = false) Boolean active) {
    ...
    @PatchMapping("/{id}")
    @PreAuthorize("hasAnyRole('AGENT', 'ADMIN')")
    public ResponseEntity<Property> patchProperty(@PathVariable Long id, @RequestBody Property property) {
        return ResponseEntity.ok(propertyService.updateProperty(id, property));
    }
    ...
    public ResponseEntity<List<Property>> searchProperties(
        ...
        return ResponseEntity.ok(propertyService.searchActiveProperties(cityId, categoryId, min, max));
    }
}
